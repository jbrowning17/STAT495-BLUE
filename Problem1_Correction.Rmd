...
barplot(table(train$PhysActive)/5984, xlab = "Physically Active", ylab = "Proportion", main = "Proportion of Physically Active")
...
The coefficients of the model for PhysActiveInactive, Age, Gendermale, Poverty, alcoholavg, and age:alcoholavg are 1.734, .0172, .307, -.224, .041, -.00974 respectively. This means that those who are NOT physically active have a BMI that is higher by 1.734 on average than those who are not physically active. For a one year age increase, after accounting for other variables, the BMI would be expected to increase by .0172. On average, males have a BMI .307 higher than females. For a one unit increase in poverty ratio, after accounting for other variables, we would expect BMI to decrease by .224. As you drink more alcohol we would expect your BMI to increase, but the older you are, the effect that alcohol has on your BMI decreases.
